...
public class OthelloBoard {
	...
	private boolean validCoordinate(int row, int col) {
        return (row >= 0) && (row <= getDimension() - 1) && (col >= 0) && (col <= getDimension() - 1);
	}
	...
	private char alternation(int row, int col, int drow, int dcol) {
		...
		char curr = get(row, col);
		while (validCoordinate(row, col) && (get(row, col) != EMPTY)) {
			if (get(row, col) == otherPlayer(curr)) {
				return otherPlayer(curr);
			}
			row += drow;
			col += dcol;
		}
	...
	public int flip(int row, int col, int drow, int dcol, char player) {
		int count = 0;
		char winner_player = alternation(row, col, drow, dcol);
		if (player == winner_player) {
			while (validCoordinate(row, col) && this.get(row, col) != winner_player) {
				count += 1;
				this.board[row][col] = winner_player;
				row += drow;
				col += dcol;
			}
			return count;
		} else if (player == otherPlayer(winner_player)) {
			return 0;
		} else {
			return -1;
		}
	...
	private char hasMove(int row, int col, int drow, int dcol) {
		if ((this.get(row, col) != EMPTY) || (!validCoordinate(row, col))){
			return EMPTY;
		}
	...
	public char hasMove() {
		...
		for (int r = 0; r < this.getDimension(); r++){
			for (int c = 0; c < this.getDimension(); c++){
				// if statement to check if r,c is empty
				if ((this.get(r, c) == EMPTY) && (validCoordinate(r, c))) {
                    for (int[] direction : DIRECTIONS) {
                        char return1 = hasMove(r, c, direction[0], direction[1]);
                        if (return1 == P1) {
                            player1 = true;
                        } else if (return1 == P2) {
                            player2 = true;
	...
	public boolean move(int row, int col, char player) {
		...
		if (this.hasMove() == player || this.hasMove() == BOTH) { //
			// turn these 2 for loops ito a a hleper that uses alternation
			for (int[] direction : DIRECTIONS) {
				char return1 = hasMove(row, col, direction[0], direction[1]);
				if (return1 == player){
	...
	/**
	 *
	 * Changes the value of (row, col) on the board
	 * @param row    starting row, in {0,...,dim-1} (typically {0,...,7})
	 * @param col    starting col, in {0,...,dim-1} (typically {0,...,7})
